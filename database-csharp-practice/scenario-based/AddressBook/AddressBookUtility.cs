...
namespace AddressBook
{
    internal class AddressBookUtility : IAddressbook 
    {
        ...
        public void WriteToCSVThreaded()  // UC-17 (MultiThreading)
        {
            Thread t = new Thread(() =>
            {
                try
                {
                    ...
                    Console.WriteLine("CSV saved in background thread.");
                }
                catch (Exception ex)
                {
                    Console.WriteLine("CSV Thread Error: " + ex.Message);
                }
            });

            t.Start(); // Start background thread
        }
        public void ReadFromCSVThreaded()  // UC-17 (MultiThreading)
        {
            Thread t = new Thread(() =>
            {
                try
                {
                    ...
                    List<AddressBook> tempList = new List<AddressBook>();
                    ...
                    // Assign after reading (avoids partial data issue)
                    addressBooks = tempList;

                    Console.WriteLine("CSV loaded in background thread.");
                }
                catch (Exception ex)
                {
                    Console.WriteLine("CSV Thread Error: " + ex.Message);
        ...
        public void WriteToJsonThreaded()   // UC-17 (MultiThreading)
        {
            Thread t = new Thread(() =>
            {
                try
                {
                    ...
                    Console.WriteLine("JSON saved in background thread.");
                }
                catch (Exception ex)
                {
                    Console.WriteLine("JSON Thread Error: " + ex.Message);
                }
            });

            t.Start();
        }
        public void ReadFromJsonThreaded()  // UC-17 (MultiThreading)
        {
            Thread t = new Thread(() =>
            {
                try
                {
                    ...
                    string json = File.ReadAllText(jsonPath);
                    List<AddressBook> tempList =
                        JsonSerializer.Deserialize<List<AddressBook>>(json);

                    addressBooks = tempList;

                    Console.WriteLine("JSON loaded in background thread.");
                }
                catch (Exception ex)
                {
                    Console.WriteLine("JSON Thread Error: " + ex.Message);
        ...
        public void SaveAddressBook()  // UC-18 (Database
        {
            dataSource.Save(addressBooks);
        }

        public void LoadAddressBook()   // UC-18 (Database)
        {
